...
check_dotfiles() {
    ...
    for directory in cava fastfetch fontconfig gtk-3.0 gtk-4.0 helix hypr kitty mako matugen mpv niri qt6ct rofi waybar zed zsh; do
...
check_services() {
    ...
    for service in "${services[@]}"; do
        if systemctl is-enabled "${service}.service" &>/dev/null; then
            pass "Service is enabled: $service"
        else
            flunk "Service is not enabled: $service (or unit not found)"
        fi
    done
